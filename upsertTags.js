const posts = require("./posts");
const fs = require("fs");
const axios = require("axios");

// const tags = posts.map((post) => post.Tags);

// async function createSetOfTags() {
//   const set = new Set();

//   for (const tag of tags) {
//     const list = tag.split(",");
//     list.forEach((t) => set.add(t));
//   }

//   console.log(Array.from(set));

//   fs.writeFile("./tags.json", JSON.stringify(Array.from(set)), (err) =>
//     console.log(err)
//   );
// }

// async function getTags() {
//   const url = "https://crossroads-backend.herokuapp.com/graphql";

//   const data = await axios.post(url, {
//     query: `query GET_TAGS {
//       tags {
//         tag
//       }
//     }`,
//   });

//   console.log((await data).data.data.tags.map((tag) => tag.tag));
// }

// getTags();

// const currentTags = [
//   "parents",
//   "classroom",
//   "training-opportunity",
//   "general",
//   "aba-clinic",
//   "applied-behavior-analysis",
//   "art",
//   "community",
//   "day-care",
//   "donations",
//   "events",
//   "fundraisers",
//   "grants",
//   "healthy-food-program",
//   "life-skills",
//   "job-skills",
//   "self-care-skills",
//   "english-language-arts",
//   "mathematics",
//   "news",
//   "physical-therapy",
//   "occupational-therapy",
//   "physical-education",
//   "science",
//   "social-skills",
//   "speech-therapy",
//   "technology",
//   "volunteering",
//   "volunteer-opportunities",
//   "psychology",
//   "at-home-activities",
//   "covid",
// ];

// function filterTags() {
//   fs.writeFile(
//     "./uniqueTags.json",
//     JSON.stringify(tags.filter((tag) => !currentTags.includes(tag))),
//     (err) => console.log(err)
//   );
// }

async function addTags() {
  const tags = [
    "autism",
    "behavior support",
    "childcare",
    "children",
    "Donations",
    "education",
    "how to help",
    "nonprofit",
    "school",
    "special needs",
    "students",
    "support",
    "volunteer",
    "activities",
    "eating",
    "eating difficulties",
    "lifeskills",
    "nutrition",
    "ABA",
    "classrooms",
    "giving",
    "SACC.TV",
    "school year",
    "social skills",
    "The Muffin Man",
    "Cock n' Bull restaurant",
    "dog",
    "pet therapy",
    "reading buddies",
    "therapy dog",
    "covid-19",
    "Speech and Language",
    "Speech therapy",
    "BCBA",
    "behavior plan",
    "calendar",
    "school calendar",
    "Prompting Procedures",
    "staff training",
    "Garden",
    "Mohawk Honda",
    "volunteers",
    "day",
    "preschool",
    "healthy food",
    "healthy food program",
    "Healthy Snack Cart",
    "integrated classrooms",
    "integrated preschool",
    "integrated program",
    "Speech",
    "Room 11",
    "Room 4",
    "sensory",
    "creativity",
    "daycare",
    "functional communication",
    "Language",
    "positive reinforcement",
    "Room 7",
    "boscov's",
    "clifton park",
    "Friends helping friends day",
    "room 14",
    "sharing",
    "nursery school",
    "ASML",
    "donors",
    "rescue",
    "Tugg",
    "whiteboards",
    "mulch",
    "physical act",
    "physical activity",
    "playground",
    "SLP",
    "speech and language therapy",
    "Grants",
    "Social Skills development",
    "healthy eating",
    "lunch time",
    "snack time",
    "play skills",
    "social interaction",
    "circle",
    "group activities",
    "Clinic",
    "jobs",
    "life skills",
    "school age",
    "self-care",
    "discrete trial training",
    "classroom themes",
    "Dr. Seuss",
    "circle time",
    "Goodbye circle",
    "learning at home",
    "Morning Circle",
    "Acts of Kindness",
    "february",
    "hearts",
    "movement",
    "physical education",
    "PT",
    "curriculum",
    "literacy",
    "LWT",
    "Centers",
    "Learning Centers",
    "themes",
    "child care",
    "day care",
    "winter",
    "",
    "autism awareness month",
    "blue hair",
    "child friendly",
    "fundraiser",
    "going blue",
    "feeding the fish",
    "focus",
    "Holiday Match Program",
    "Stewart's Shops",
    "Cock n' Bull",
    "Scott Sherrard",
    "preference assessment",
    "reinforcers",
    "Speech sessions",
    "community support",
    "tattoo",
    "tattoos",
    "play building blocks",
    "LifeSong",
    "SEFA",
    "grant",
    "Kvistad Foundation",
    "Woodstock Chimes Fund",
    "Autism awareness",
    "prevalence",
    "Earth Day",
    "Earth Week",
    "Recycling",
    "music",
    "Singalong",
    "group lesson",
    "Better Hearing and Speech Month",
    "Speech Language Pathologists",
    "building",
    "toys",
    "Volunteer Appreciation",
    "Dover",
    "friesen",
    "Jessica Friesen",
    "pocono",
    "puzzle pices",
    "racecar",
    "raffle",
    "Stewart Friesen",
    "iPad program",
    "iPads",
    "Ronald McDonald House Charities",
    "SEFCU",
    "Allen Foundation",
    "Heatlhy food program",
    "Every Kid Healthy Week",
    "Healthy Kids Celebration",
    "Blood Drive",
    "Red Cross",
    "donate",
    "Wright Family Foundation",
    "recurring donations",
    "the Christopher Dailey Foundation",
    "fish theme",
    "Room 1",
    "2019",
    "Graduation",
    "careers",
    "certification",
    "staff",
    "Team",
    "Back to School",
    "welcome back",
    "first day of school",
    "enewsletter sign up",
    "newsletter",
    "social channels",
    "social media",
    "Shenendehowa Central Schools",
    "teeshirts",
    "2019 golf",
    "golf tournament",
    "playground mulching",
    "Schenectady County Legislature",
    "Schenectady County Recycling Center",
    "Back to School Night",
    "Parents",
    "#GivingTuesday",
    "#NYGivesDay",
    "giving tuesday",
    "Ballroom Dance Week",
    "Dance Fire Studio",
    "2018",
    "Speech therapists",
    "AUCTIONS",
    "BENEFITS",
    "gala",
    "GALA 2020",
    "Room 5",
    "Room 8",
    "Handwriting",
    "jumping",
    "OT",
    "stairs",
    "library",
    "reading",
    "owls",
    "Room 3",
    "Room 6",
    "bottles and cans",
    "recycle",
    "Goodbye song",
    "how to make a difference",
    "ways to make a difference",
    "after school",
    "before school",
    "cleanup",
    "fall",
    "playground safety",
    "play",
    "Red Hat Ladies",
    "CDPHP",
    "special education",
    "Special Events",
    "books",
    "routines",
    "visual schedules",
    "Ann Parillo",
    "Schenectady Today",
    "TV Show",
    "Action for Healthy Kids grant",
    "hoops",
    "PE",
    "social stories",
    "trials",
    "Thanksgiving",
    "turkey feathers",
    "Give Gab",
    "Go Fund Me",
    "Clynk",
    "fundraising",
    "projects",
    "Kid Cents",
    "Kid Cents Program",
    "Rite Aid Kid Cents",
    "DASA",
    "training",
    "Henry E. Niles",
    "Graphing training",
    "professional development",
    "Annual campaign",
    "charitable giving",
    "end of the year giving",
    "learning",
    "settings",
    "Christmas",
    "Elf",
    "Santa",
    "concerts",
    "20 years",
    "20th anniversary",
    "anniversary",
    "Honest Weight Food Cop-op",
    "FAQs",
    "frequently asked questions",
    "gala 2019",
    "Hannaford Helps",
    "ROTTERDAM",
    "crossroads",
    "appreciation",
    "bingo",
    "colors",
    "dads",
    "lookcrossroads",
    "moms",
    "pets",
    "school spirit",
    "golf 2019",
    "Cart Sponsors",
    "Hole Sponsors",
    "Sponsors",
    "Kids Connections Early Learning",
    "shade",
    "spring appeal",
    "fitness",
    "special olympics",
    "young atletes",
    "nora roberts foundation",
    "ABA training",
    "preference assessments",
    "Regeneron",
    "STEM",
    "Halloween parades.",
    "defibrillator",
    "art supplies",
    "Price Chopper",
    "tools for schools",
    "home learning",
    "task analysis",
    "corona virus",
    "store",
    "tees",
    "AIM",
    "VISUAL SUPPORTS",
    "NET",
    "stewart's/dake family",
    "planned ignoring",
    "matching donations",
    "Bing to Give",
    "parent meetings",
    "radio interview",
    "wamc",
    "Gofundme",
    "ride it forward",
    "amazon",
    "butterfly",
    "grant. butterflies",
    "secret reader",
    "halloween",
    "pumpkins",
    "Arnold Cogswell Fund",
    "Community Foundation",
    "covid safety",
    "festival of trees",
    "thanks",
  ];

  for (const tag of tags) {
    const url = "https://crossroads-backend.herokuapp.com/tags";
    await axios.post(url, { tag });
  }
}

addTags();
